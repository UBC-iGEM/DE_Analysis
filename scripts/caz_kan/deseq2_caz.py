...
if STANDARDIZED_COUNTS.exists() and STANDARDIZED_METADATA.exists():
    counts, metadata = load_standardized_counts('ceftazidime')
else:
    ...
    metadata = pd.DataFrame({
        'condition': ['ceftazidime']*len(caz.columns) + ['control']*len(wu.columns)
    }, index=counts.index)
...
stats = DeseqStats(
    dds,
    contrast=['condition', 'ceftazidime', 'control'],
    inference=inference,
    n_cpus=1,
)
stats.summary()
